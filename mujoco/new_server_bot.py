...
class NewServerBotEnv(gym.GoalEnv):
    def __init__(self, model_path="Adroit/new_env.xml", frame_skip=5):
        ...
        goal = self.sim.data.geom_xpos[self.sim.model.geom_names.index("target")]
        self.init_height = goal[axis_idx['h']].copy()
        self.reward_type = "sparse"
        right_wrist = self.sim.data.body_xpos[self.sim.model.body_names.index("wrist")]
        right_middle = self.sim.data.body_xpos[self.sim.model.body_names.index("mfknuckle")]
        self.init_center_pos = (right_wrist + right_middle) / 2
    ...
    def _set_action_space(self):
        model = self.model
        self.joint_names = ["right_shoulder_1", "right_shoulder_2", "right_elbow_1", "right_elbow_2", "right_elbow_3",
                            "WRJ1", "WRJ0", "left_shoulder_1", "left_shoulder_2", "left_elbow_1", "left_elbow_2",
                            "left_elbow_3"]
        ...
        self.action_space = spaces.Box(low=np.array(low_bound, dtype=np.float32), high=np.array(high_bound, dtype=np.float32), dtype=np.float32)
    ...
    def _get_obs(self):
        ...
        return {
            ...
            "achieved_goal": obs[obs_idx["R_CT"]: obs_idx["R_CT"] + 3].copy(),
            "desired_goal": obs[obs_idx["T"]: obs_idx["T"] + 3].copy(),
        }
    ...
    def touched(self, achieved_goal, goal):
        model = self.sim.model
        target_pos = goal * self.obs_bound
        center_pos = achieved_goal * self.obs_bound
        fractor = 1.2
        return self.get_dist(center_pos / self.obs_bound, goal) <= fractor * model.geom_size[model.geom_names.index("target")][0] \
               and center_pos[axis_idx['w']] < target_pos[axis_idx['w']] \
               and center_pos[axis_idx['h']] < (target_pos[axis_idx['h']] - 0.1)

    def get_dist(self, achieved_goal, goal):
        target_pos = goal * self.obs_bound
        center_pos = achieved_goal * self.obs_bound
        return np.linalg.norm(target_pos - center_pos, axis=-1)

    def compute_reward(self, achieved_goal, goal, info=None):
        d = self.get_dist(achieved_goal, goal)
        return -(d > self.distance_threshold).astype(np.float32)

    def _sample_goal(self):
        return self.init_center_pos.copy()

    def _is_success(self, achieved_goal, desired_goal):
        return self.touched(achieved_goal, desired_goal)
    ...
    def step(self, a):
        ...
        action[a.size - 1] = 0
        discount = 1
        action *= discount  # slow down the move of bot
        self.do_simulation(action, self.frame_skip)
        self.macro_step(action, macro_action)
        next_obs = self._get_obs()
        if not self.in_desk():
            return next_obs, LOSE, True, {
                "is_success": self._is_success(next_obs["achieved_goal"], next_obs["desired_goal"]),
            }
        if self.touched(next_obs["achieved_goal"], next_obs["desired_goal"]):
